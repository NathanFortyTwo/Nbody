...
void run_simulation()
{
  ...
  while (t < T_FINAL && nparticles > 0)
  {
    ...
    MPI_Barrier(MPI_COMM_WORLD); // on doit attendre que les particules soient au bon endroi
...
void broadcast()
{
  ...
  double allXpos[nparticles];
  double allYpos[nparticles];

  int remainder = nparticles % 4;
  int start = rank * nparticles / 4;
  int end = (rank + 1) * nparticles / 4;
  int local_nbparticules = end - start;

  if (rank == 4)
  {
    end += remainder;
    local_nbparticules += remainder;
  }

  double *Xpos = malloc(sizeof(double) * local_nbparticules);
  double *Ypos = malloc(sizeof(double) * local_nbparticules);

  for (int i = start; i < end; i++)
  {
    particle_t particle = particles[i];
    Xpos[i - start] = particle.x_pos;
    Ypos[i - start] = particle.y_pos;
  }
  // ATTENTION NBPARTICULES N'EST PAS FORCEMENT UN MULTIPLE DE 4
  // DONC ON DOIT FAIRE UN GATHERV DONC CHANGER LES DEUX LIGNES SUIVANTES
  MPI_Allgather(Xpos, local_nbparticules, MPI_DOUBLE, allXpos, local_nbparticules, MPI_DOUBLE, MPI_COMM_WORLD);
  MPI_Allgather(Ypos, local_nbparticules, MPI_DOUBLE, allYpos, local_nbparticules, MPI_DOUBLE, MPI_COMM_WORLD);
}
